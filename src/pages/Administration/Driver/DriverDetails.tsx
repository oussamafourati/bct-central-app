...
import { Button, Card, Col, Dropdown, Container, Row, Table, Modal, Tab, Nav } from 'react-bootstrap';
      ...
      <div className="page-content">
          ...
          <Card.Body>
            <Row>
              <div className='hstack gap-0'>
                <Col lg={3}>
                  <div className="profile-user-img position-relative" style={{ width: "400px" }}>
                    <img src={`http://localhost:3000/driverFiles/profileImages/${driverLocation.state.profile_image}`} alt="" className="rounded cover" />
                ...
                <Col lg={9}>
                  <div className="d-flex border-bottom border-bottom-dashed mb-0 mt-10 mt-lg-4">
                    <div className="flex-grow-5 mb-2">
                      <h4>{driverLocation.state.firstname} {driverLocation.state.surname}</h4>
                      {driverLocation.state.driverStatus === "Active" ? <span className="badge bg-success">Active</span> : driverLocation.state.driverStatus === "Inactive" ? <span className="badge bg-danger">Inactive</span> : driverLocation.state.driverStatus === "onRoad" ? <span className="badge bg-info">On Road</span> : <span className="badge bg-warning">On Vacation</span>}
                    </div>
                  </div>
            ...
            <Row>
              <div className='text-center hstack gap-5'>
                <Button variant='soft-danger' className="btn-label" onClick={() => { tog_AddShippingModals(); }}><i className="bi bi-filetype-pdf label-icon align-middle fs-24 me-2"></i> Driving License</Button>
                <Button variant='soft-danger' className="btn-label" onClick={() => { tog_DQCModal(); }}><i className="bi bi-filetype-pdf label-icon align-middle fs-24 me-2"></i> DQC</Button>
                <Button variant='soft-danger' className="btn-label" onClick={() => { tog_DBSModals(); }}><i className="bi bi-filetype-pdf label-icon align-middle fs-24 me-2"></i> DBS</Button>
                <Button variant='soft-danger' className="btn-label" onClick={() => { tog_PVCModals(); }}><i className="bi bi-filetype-pdf label-icon align-middle fs-24 me-2"></i> PVC</Button>
              </div>
            </Row>
          </Card.Body>
        </Card.Body>
        <Modal className="fade zoomIn" size="xl" show={modal_AddShippingModals} onHide={() => { tog_AddShippingModals(); }} centered>
          <Modal.Header className="px-4 pt-4" closeButton>
            <h5 className="modal-title fs-18" id="exampleModalLabel">Driving License</h5>
          </Modal.Header>
          <Modal.Body className="p-4">
            <div id="alert-error-msg" className="d-none alert alert-danger py-2"></div>
            <div>
              <Document file={`http://localhost:3000/driverFiles/licenseFiles/${driverLocation.state.driver_license}`} onLoadSuccess={onDocumentLoadSuccess}>
                <Page pageNumber={1} />
              </Document>
            </div>
          </Modal.Body>
        ...
        <Modal className="fade zoomIn" size="xl" show={modal_DQCModal} onHide={() => { tog_DQCModal(); }} centered>
          <Modal.Header className="px-4 pt-4" closeButton>
            <h5 className="modal-title fs-18" id="exampleModalLabel">DQC</h5>
          </Modal.Header>
          <Modal.Body className="p-4">
            <div id="alert-error-msg" className="d-none alert alert-danger py-2"></div>
            <div>
              <Document file={`http://localhost:3000/driverFiles/dqcFiles/${driverLocation.state.dqc}`} onLoadSuccess={onDocumentLoadSuccess}>
                <Page pageNumber={1} />
              </Document>
            </div>
          </Modal.Body>
        ...
        <Modal className="fade zoomIn" size="xl" show={modal_DBSModals} onHide={() => { tog_DBSModals(); }} centered>
          <Modal.Header className="px-4 pt-4" closeButton>
            <h5 className="modal-title fs-18" id="exampleModalLabel">DBS</h5>
          </Modal.Header>
          <Modal.Body className="p-4">
            <div id="alert-error-msg" className="d-none alert alert-danger py-2"></div>
            <div>
              <Document file={`http://localhost:3000/driverFiles/dbsCheckFiles/${driverLocation.state.dbscheck}`} onLoadSuccess={onDocumentLoadSuccess}>
                <Page pageNumber={1} />
              </Document>
            </div>
          </Modal.Body>
        ...
        <Modal className="fade zoomIn" size="xl" show={modal_PVCModals} onHide={() => { tog_PVCModals(); }} centered>
          <Modal.Header className="px-4 pt-4" closeButton>
            <h5 className="modal-title fs-18" id="exampleModalLabel">PVC</h5>
          </Modal.Header>
          <Modal.Body className="p-4">
            <div id="alert-error-msg" className="d-none alert alert-danger py-2"></div>
            <div>
              <Document file={`http://localhost:3000/driverFiles/contractFiles/${driverLocation.state.contract}`} onLoadSuccess={onDocumentLoadSuccess}>
                <Page pageNumber={1} />
              </Document>
            </div>
          </Modal.Body>
